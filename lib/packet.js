...
function buildHighlightedFiles(repoRoot, files) {
  return files.map((file) => {
    ...
    return {
      path: file,
      snippet: summarizeHighlightedContent(file, content)
    };
  });
}
...
function summarizeHighlightedContent(filePath, content) {
  ...
  if (/\.(js|ts|jsx|tsx|vue|php)$/i.test(extension)) {
    return summarizeCodeContent(filePath, content);
  }

  if (/\.(md|markdown|txt|yml|yaml|json)$/i.test(extension)) {
    return compressText(content.slice(0, 2400), 'balanced', { type: 'rules', maxLines: 24 }).output;
  }

  return compressText(content.slice(0, 1800), 'balanced', { maxLines: 20 }).output;
}

function summarizeCodeContent(filePath, content) {
  ...
  for (const line of lines) {
    ...
    if (/^(import\s.+|const\s.+?=\s*require\(|use\s+[A-Za-z0-9_\\]+;)/.test(trimmed)) {
      imports.push(trimmed);
      continue;
    }

    const exportMatch = trimmed.match(/^(export\s+(default\s+)?(?:async\s+)?(?:function|class|const|let|var)\s+([A-Za-z0-9_]+)|module\.exports\s*=|exports\.([A-Za-z0-9_]+)\s*=)/);
    if (exportMatch) {
    ...
    if (/^(const|let|var)\s+[A-Z0-9_]{3,}\s*=/.test(trimmed)) {
      constants.push(trimmed);
    }
  }

  const summaryLines = [`File: ${filePath}`];
  appendGroup(summaryLines, 'Imports', imports, 4);
  appendGroup(summaryLines, 'Exports', exportsList, 4);
  appendGroup(summaryLines, 'Classes', classes, 4);
  appendGroup(summaryLines, 'Functions', functions, 8);
  appendGroup(summaryLines, 'Constants', constants, 4);
