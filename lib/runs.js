...
function showRunCommand(options, cwd) {
  ...
  const store = getRepoStore(repoRoot);
  const runs = loadRuns(path.join(store, 'runs'));
  const selectedRunId = String(options.runId || '').trim();
  let run = null;

  if (!selectedRunId || selectedRunId === 'latest') {
    run = runs[runs.length - 1];
  } else {
    run = runs.find((item) => item.id === selectedRunId);
  }
...
function renderStatus(status, format, options = {}) {
  ...
  if (status.latestPacket) {
    lines.push(`Last packet reduction: ${formatPacketReduction(status.latestPacket)}`);
  }
...
function formatPacketReduction(packet) {
  if (!packet || !packet.diffTokens) {
    return 'No diff to compress';
  }
  return `${packet.reductionPercent}%`;
}
