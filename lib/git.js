...
function getChangedFiles(repoRoot, options) {
  ...
  const staged = getStagedNameOnly(repoRoot);
  const worktree = getWorktreeNameOnly(repoRoot);
  const untracked = getUntrackedFiles(repoRoot);
  const changed = []
    .concat(committed ? committed.split('\n').filter(Boolean) : [])
    .concat(staged ? staged.split('\n').filter(Boolean) : [])
...
function getDiff(repoRoot, options, files) {
  ...
  const staged = getStagedDiff(repoRoot, files);
  const worktree = getWorktreeDiff(repoRoot, files);
  return [committed, staged, worktree].filter(Boolean).join('\n');
}
...
function getStagedNameOnly(repoRoot) {
  try {
    return runGit(repoRoot, ['diff', '--cached', '--name-only']);
  } catch (error) {
    return '';
  }
}

function getStagedDiff(repoRoot, files) {
  const args = ['diff', '--cached', '--unified=1'];
  if (files && files.length) {
    args.push('--', ...files);
  }
